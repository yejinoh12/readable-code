...
public class MinesweeperGame {
    ...
    private static void actOnCell(String cellInput, String userActionInput) {
        ...
        if (doesUserChooseToOpenCell(userActionInput)) {
            if (isLandMineCell(selectedRowIndex, selectedColIndex)) { //보드의 지뢰 여부 판단
    ...
    private static boolean isLandMineCell(int selectedRowIndex, int selectedColIndex) {
        return BOARD[selectedRowIndex][selectedColIndex].isLandMain();
    }
    ...
    private static int getSelectedColIndex(String cellInput) {
        ...
        return covertColFrom(cellInputCol);
    }
    ...
    private static boolean IsAllCellChecked() {
        return Arrays.stream(BOARD)
                ...
                .allMatch(Cell::isChecked); //모든 셀이 다 체크 되었는지 확인
    }
    ...
    private static int covertColFrom(char cellInputCol) {
        switch (cellInputCol) {
            case 'a':
                return 0;
            case 'b':
    ...
    private static void initializeGame() {
        for (int row = 0; row < BOARD_ROW_SIZE; row++) {
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
                BOARD[row][col] = Cell.create(); //셀 생성
            }
        }
        ...
        for (int i = 0; i < LAND_MINE_COUNT; i++) {
            ...
            BOARD[row][col].turnOnLandMine(); //지뢰 설정
        }
    ...
    private static void open(int row, int col) {
        ...
        if (BOARD[row][col].isOpened()) { //보드가 이미 열려 있다면 return
            return;
        }
        if (isLandMineCell(row, col)) { //지뢰 셀이라면 return
            return;
        }

        BOARD[row][col].open(); //셀 오픈
        if (BOARD[row][col].hasLandMineCount()) { //오픈 된 셀이 숫자 셀이면 return
            return;
        }
